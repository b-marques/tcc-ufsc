...
\todo[inline]{Márcio, eu peguei os dados com o Emmanuel e quando verifiquei, eu vi os dados referente ao teste de escalabilidade que ele fez pro TCC dele, aonde eram executada 30 iterações deduzi que os outros dados (referente ao tcc dele)também estariam corretos, porém quando fui pegar os resultados referentes as execuções das aplicações, lá só aparece os valores gerados para aquele artigo da ERAD com apenas 1 iteração de comparação entre IPC e ASYNC, eu mandei mensagem pra ele e estou esperando a resposta ainda. Só verifiquei a falta desses dados de sexta pra sábado, quando eu fui fazer os gráficos de comparação pra adicionar ao texto, eu tentei executar a versão dele no mppa, mas sem sucesso, vários problemas de memória que eu não consegui debugar de maneira paleativa}

Esta seção apresenta a comparação entre a nova versão do \pskelmppa (\async) com a versão antiga do \pskelmppa (IPC). A Figura~\ref{sec:async_vs_ipc} apresenta a comparação de tempo de execução e consumo de energia entre estas versões. Nesse experimento, foi utilizada uma matriz de dados de tamanho \ind, \tiles de tamanho \tilec e \tiled e 16 \clusters. O número de iterações utilizado foi $t = 30$. Os resultados obtidos representam a media de 5 execuções com o desvio padrão médio menor que $1\%$, este número de execuções visa manter paridade com os resultados gerados na versão \ipc~\cite{Podesta:TCC}. Como pode ser observado,  os resultados mostram que o tempo de execução da versão \async é, aproximadamente, \todo[inline]{xx} menor em relação ao tempo da versão \ipc. Essa diferença está correlacionada ao fato da versão \async fazer melhor uso da \noc na distribuição dos dados para os \cluster de computação e realizar menos sincronizações entre mestre e trabalhador. Além disso, otimizações na computação como a verificação de elementos presentes nas bordas de computação também contribuem para este resultado.
O resultado da energia consumida segue um comportamento similar, uma vez que, menos tempo executando acarreta em menos tempo consumindo energia. Assim, é apresentada uma eficiência no consumo de energia superior em até \todo[inline]{xx} a favor da versão \async.
